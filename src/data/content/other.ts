import type { ContentItem } from "./types";

export const otherData: ContentItem[] = [
  {
    id: "animation-on-the-web",
    title: "Animation on the web",
    description: "Courses about animation on the web.",
    link: "https://animations.dev/learn",
    category: "other",
    tags: ["animation", "courses"],
    type: "resource",
    resourceType: "tutorial",
  },
  {
    id: "blend-modes",
    title: "Blend Modes",
    description: "Playground for exploring blend modes.",
    link: "https://blendmodes.dev/multiply",
    category: "other",
    tags: ["blend modes", "colors playground"],
    type: "resource",
    resourceType: "tool",
  },
  {
    id: "ui-patterns",
    title: "UI Patterns",
    description: "Collection of UI design patterns and design strategies.",
    link: "https://ui-patterns.com/",
    category: "other",
    tags: ["ui design patterns"],
    type: "resource",
    resourceType: "tutorial",
  },
  {
    id: "poline",
    title: "Poline",
    description: "An enigmatic color palette generator, that harnesses the mystical witchcraft of polar coordinates.",
    link: "https://meodai.github.io/poline/",
    category: "other",
    tags: ["color system", "color palette"],
    type: "resource",
    resourceType: "tool",
  },
  {
    id: "modalzmodalzmodalz",
    title: "Modalzmodalzmodalz",
    description: "A simple design pattern about using modals.",
    link: "https://modalzmodalzmodalz.com/",
    category: "other",
    tags: ["modal insights", "design pattern"],
    type: "resource",
    resourceType: "other",
  },
  {
    id: "componentsbuild",
    title: "Components Build",
    description: "A handbook for building UI components.",
    link: "https://www.components.build/",
    category: "other",
    tags: ["components", "handbook"],
    type: "resource",
    resourceType: "tutorial",
  },
];
